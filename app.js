...
function initLanding() {
  ...
  const mobileFlowQuery =
    typeof window !== "undefined" && typeof window.matchMedia === "function"
      ...
      : null;
  const isMobileFlow = () => (mobileFlowQuery ? mobileFlowQuery.matches : false);
  ...
  function setFlowStep(step) {
    ...
    const previousIndex = flowSteps.indexOf(currentFlowStep);
    const targetIndex = flowSteps.indexOf(step);
    currentFlowStep = step;
    const useCarousel = isMobileFlow();

    flowTrack.dataset.currentStep = step;
    flowTrack.dataset.direction = useCarousel
    ...
    flowSteps.forEach((name, index) => {
      ...
      if (!panel) return;
      const isActive = useCarousel ? name === step : true;
      panel.classList.toggle("is-active", isActive);
      panel.classList.toggle("is-before", useCarousel && index < targetIndex);
      panel.classList.toggle("is-after", useCarousel && index > targetIndex);
      panel.setAttribute(
        "aria-hidden",
        isActive ? "false" : useCarousel ? "true" : "false"
      );
    });
    ...
    if (useCarousel) {
      window.requestAnimationFrame(() => {
        const activePanel = flowTrack.querySelector(`[data-step="${step}"]`);
        if (activePanel) {
          flowViewport.style.height = `${activePanel.offsetHeight}px`;
        }
      });
    } else {
      flowViewport.style.height = "";
    }
  }
  ...
  if (mobileFlowQuery) {
    const listener = () => handleLayoutShift();
    if (typeof mobileFlowQuery.addEventListener === "function") {
      mobileFlowQuery.addEventListener("change", listener);
    } else if (typeof mobileFlowQuery.addListener === "function") {
      mobileFlowQuery.addListener(listener);
    }
  }
